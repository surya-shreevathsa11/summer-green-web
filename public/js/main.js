(function () {
  ...
  function registerReveals(root = document) {
    ...
    els.forEach((el) => {
      ...
      if (type === 'slide-down' || type === 'flow') {
        el.classList.add(type === 'slide-down' ? 'reveal-slide-down' : 'reveal-flow');
      } else {
  ...
  $$('.nav__links a, .hero .btn, .footer__links a, .section__actions a').forEach(link => {
  ...
  async function renderRooms() {
    try {
      ...
      grid.innerHTML = data.rooms.map((room, idx) => `
  ...
  function setupCursor() {
    ...
    window.addEventListener('mousedown', () => { down = true; updateRingClasses(); });
    window.addEventListener('mouseup', () => { down = false; updateRingClasses(); });

    document.addEventListener('mouseover', (e) => {
      const target = e.target && e.target.closest && e.target.closest('a, button, .btn, input, textarea, [role="button"]');
    ...
    document.addEventListener('mouseout', (e) => {
      ...
      const stillHover = e.relatedTarget.closest && e.relatedTarget.closest('a, button, .btn, input, textarea, [role="button"]');
